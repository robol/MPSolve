 ...
 */
void
mps_mandelbrot_poly_fnewton (mps_context * s, mps_polynomial * poly, mps_approximation * root, cplx_t corr)
{
  cplx_t p, pp, pt, tmp;
  double ap, ax, eps;
 ...
 */
void
mps_mandelbrot_poly_dnewton (mps_context * s, mps_polynomial * poly, mps_approximation * root, cdpe_t corr)
{
  cdpe_t p, pp, pt, tmp;
  ...
  rdpe_mul_eq_d (root->drad, (double) n);
  cdpe_mod (temp, pp);
...
{
  int i, m, n = poly->degree;
  rdpe_t ap, ax, eps, temp, apeps, atmp, epsilon;
  cdpe_t ctmp;
  mpc_t p, pp, pt, tmp;
