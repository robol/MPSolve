...
mps_outroot (mps_context * s, int i, int num)
{
  ...
  switch (s->output_config->format)
    ...
    case MPS_OUTPUT_FORMAT_OPENSCAD:
      fprintf(s->outstr, "  translate([ ");
      break;
...
mps_output (mps_context * s)
{
  ...
  /* If the output is OpenSCAD, start with a header describing this file. */
  if (s->output_config->format == MPS_OUTPUT_FORMAT_OPENSCAD)
    {
      fprintf (s->outstr, "// This file has been generated by MPSolve. \n");
      fprintf (s->outstr, "// \n");
      fprintf (s->outstr, "// It contains a 3D model of the roots of a polynomial of degree %d \n", s->n);
      fprintf (s->outstr, "\n");

      /* General configuration for the size of points, and the height of
       * the extrusion. This can be set by the user later on. */
      fprintf (s->outstr, "// Adjust these options to change the appearance of the print\n");
      fprintf (s->outstr, "extruded_height = 0.4;\n");
      fprintf (s->outstr, "root_radius     = 0.25;\n");
      fprintf (s->outstr, "$fn = 25; // Higher values yield better circles\n");

      fprintf (s->outstr, "\nlinear_extrude(height = extruded_height) {\n");
    }
